...
art boasts a clean HTML5 structure with extensible CSS classes and ID's for unlimited
...
art's goal is to provide themers the building blocks needed to get their designs up and
running quickly and simply.

art is perfect if you want a simple, smart, and flexible theme starter.
...
art utilizes SASS for adaptive grids and layouts and general structure of the site.
...
art is meant to be YOUR theme. To change the name of the theme from 'art' to another name like 'mytheme',
follow these steps (to do BEFORE enabling the theme) :

  - rename the theme folder to 'mytheme'
  - rename art.info to mytheme.info
  - Edit art.info and change the name, description, project (can be deleted), replace all other instances of "art" ie. [art_tabs] would become [mytheme_tabs]
  - In template.php change each iteration of 'art' to 'mytheme'
  - In theme-settings.php change each iteration of 'art' to 'mytheme'
...
- art.info                => provide informations about the theme, like regions, css, settings, js ...
...
The layout used in art is fairly similar to the Holy Grail method. It has been tested on
...
This is how the page template is buit in art, and it works in fluid and fixed layout.
...
In order to use grunt (http://gruntjs.com/) with art you will need to install a few programs.
These will only be installed the first time you start a project using art with Grunt.
...
Once you start using art, you will massively change it until a point where it has nothing
to do with art anymore. Unlike ZEN, art is not intended to be use as a base theme for a
sub-theme (even though it is possible to do so). Because of this, it is not necessary to
update your theme when a new version of art comes out. Always see art as a STARTER, and
as soon as you start using it, it is not art anymore, but your own theme.

If you didn't rename your theme, but you don't want to be notified when art has a new version
by the update module, simply delete "project = "art" in art.info

__________________________________________________________________________________________

Thanks for using art, and remember to use the issue queue in drupal.org for any question
or bug report:

http://drupal.org/project/issues/art
